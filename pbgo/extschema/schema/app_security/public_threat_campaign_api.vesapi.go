...
// Create ThreatCampaignAPI GRPC Client satisfying server.CustomClient
type ThreatCampaignAPIGrpcClient struct {
	conn       *grpc.ClientConn
	grpcClient ThreatCampaignAPIClient
	// map of rpc name to its invocation
	rpcFns map[string]func(context.Context, string, ...grpc.CallOption) (proto.Message, error)
}

func (c *ThreatCampaignAPIGrpcClient) doRPCGetThreatCampaignById(ctx context.Context, yamlReq string, opts ...grpc.CallOption) (proto.Message, error) {
	req := &ThreatCampaignRequest{}
	if err := codec.FromYAML(yamlReq, req); err != nil {
		return nil, fmt.Errorf("YAML Request %s is not of type *ves.io.schema.app_security.ThreatCampaignRequest", yamlReq)
	}
...
func NewThreatCampaignAPIGrpcClient(cc *grpc.ClientConn) server.CustomClient {
	ccl := &ThreatCampaignAPIGrpcClient{
		conn:       cc,
		grpcClient: NewThreatCampaignAPIClient(cc),
	}
...
// Create ThreatCampaignAPI REST Client satisfying server.CustomClient
type ThreatCampaignAPIRestClient struct {
	baseURL string
	client  http.Client
	// map of rpc name to its invocation
...
func (c *ThreatCampaignAPIRestClient) doRPCGetThreatCampaignById(ctx context.Context, callOpts *server.CustomCallOpts) (proto.Message, error) {
	if callOpts.URI == "" {
	...
	req := &ThreatCampaignRequest{}
	if err := codec.FromYAML(yamlReq, req); err != nil {
		return nil, fmt.Errorf("YAML Request %s is not of type *ves.io.schema.app_security.ThreatCampaignRequest: %s", yamlReq, err)
	}
...
func NewThreatCampaignAPIRestClient(baseURL string, hc http.Client) server.CustomClient {
	ccl := &ThreatCampaignAPIRestClient{
		baseURL: baseURL,
		client:  hc,
	}
...
// Create threatCampaignAPIInprocClient

// INPROC Client (satisfying ThreatCampaignAPIClient interface)
type threatCampaignAPIInprocClient struct {
	ThreatCampaignAPIServer
}
...
func NewThreatCampaignAPIInprocClient(svc svcfw.Service) ThreatCampaignAPIClient {
	return &threatCampaignAPIInprocClient{ThreatCampaignAPIServer: NewThreatCampaignAPIServer(svc)}
}

// RegisterGwThreatCampaignAPIHandler registers with grpc-gw with an inproc-client backing so that
// rest to grpc happens without a grpc.Dial (thus avoiding additional certs for mTLS)
func RegisterGwThreatCampaignAPIHandler(ctx context.Context, mux *runtime.ServeMux, svc interface{}) error {
	s, ok := svc.(svcfw.Service)
	if !ok {
		return fmt.Errorf("svc is not svcfw.Service")
	}
	return RegisterThreatCampaignAPIHandlerClient(ctx, mux, NewThreatCampaignAPIInprocClient(s))
}

// Create threatCampaignAPISrv

// SERVER (satisfying ThreatCampaignAPIServer interface)
type threatCampaignAPISrv struct {
	svc svcfw.Service
}

func (s *threatCampaignAPISrv) GetThreatCampaignById(ctx context.Context, in *ThreatCampaignRequest) (*ThreatCampaign, error) {
	...
	bodyFields := svcfw.GenAuditReqBodyFields(ctx, s.svc, "ves.io.schema.app_security.ThreatCampaignRequest", in)
	defer func() {
		if len(bodyFields) > 0 {
			server.ExtendAPIAudit(ctx, svcfw.PublicAPIBodyLog.Uid, bodyFields)
		}
		userMsg := "The 'ThreatCampaignAPI.GetThreatCampaignById' operation on 'app_security'"
		if err == nil {
	...
	if s.svc.Config().EnableAPIValidation {
		if rvFn := s.svc.GetRPCValidator("ves.io.schema.app_security.ThreatCampaignAPI.GetThreatCampaignById"); rvFn != nil {
	...
	rsp, err = cah.GetThreatCampaignById(ctx, in)
	if err != nil {
...
func NewThreatCampaignAPIServer(svc svcfw.Service) ThreatCampaignAPIServer {
	return &threatCampaignAPISrv{svc: svc}
}

var ThreatCampaignAPISwaggerJSON string = `{
    "swagger": "2.0",
    "info": {
    ...
    "x-ves-proto-file": "ves.io/schema/app_security/public_threat_campaign_api.proto"
}`
