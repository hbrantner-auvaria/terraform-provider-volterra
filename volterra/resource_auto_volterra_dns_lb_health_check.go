	...
	if v, ok := d.GetOk("http_health_check"); ok && !isIntfNil(v) && !healthCheckTypeFound {
		...
		for _, set := range sl {
			if set != nil {
				...
				virtualHostChoiceTypeFound := false

				if v, ok := cs["disable_virtual_host"]; ok && !isIntfNil(v) && !virtualHostChoiceTypeFound {
				...
				if v, ok := cs["virtual_host"]; ok && !isIntfNil(v) && !virtualHostChoiceTypeFound {
					...
					virtualHostChoiceTypeFound = true
					virtualHostChoiceInt := &ves_io_schema_dns_lb_health_check.HttpHealthCheck_VirtualHost{}

					healthCheckInt.HttpHealthCheck.VirtualHostChoice = virtualHostChoiceInt

					virtualHostChoiceInt.VirtualHost = v.(string)
