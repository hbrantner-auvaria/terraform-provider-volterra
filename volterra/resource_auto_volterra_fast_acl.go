										...
										if v, ok := cs["address"]; ok && !isIntfNil(v) {
											...
											for i, set := range sl {
												if set != nil {
													...
													if v, ok := addressMapStrToI["dual_stack"]; ok && !isIntfNil(v) && !verTypeFound {
...
func resourceVolterraFastAclDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = client.DeleteObject(context.Background(), ves_io_schema_fast_acl.ObjectType, namespace, name, opts...)
	if err != nil {
		return fmt.Errorf("error deleting FastAcl: %w", err)
	}
	return nil
