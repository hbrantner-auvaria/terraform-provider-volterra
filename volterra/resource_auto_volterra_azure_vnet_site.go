...
func resourceVolterraAzureVnetSiteCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("disable_encryption"); ok && !encryptionChoiceTypeFound {
		...
		if v.(bool) {
			encryptionChoiceInt := &ves_io_schema_views_azure_vnet_site.CreateSpecType_DisableEncryption{}
	...
	if v, ok := d.GetOk("enable_encryption"); ok && !isIntfNil(v) && !encryptionChoiceTypeFound {
		...
		encryptionChoiceTypeFound = true
		encryptionChoiceInt := &ves_io_schema_views_azure_vnet_site.CreateSpecType_EnableEncryption{}
		encryptionChoiceInt.EnableEncryption = &ves_io_schema_views_azure_vnet_site.AzureDiskEncryption{}
		createSpec.EncryptionChoice = encryptionChoiceInt
		...
		for _, set := range sl {
			if set != nil {
				...
				if v, ok := cs["disk_encryption_set_id"]; ok && !isIntfNil(v) {

					encryptionChoiceInt.EnableEncryption.DiskEncryptionSetId = v.(string)

				}

				if v, ok := cs["resource_group"]; ok && !isIntfNil(v) {

					encryptionChoiceInt.EnableEncryption.ResourceGroup = v.(string)
