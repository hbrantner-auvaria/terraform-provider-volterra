...
func resourceVolterraGcpVpcSite() *schema.Resource {
	return &schema.Resource{
		...
		StateUpgraders: []schema.StateUpgrader{
			{
				Type:    statemigration.ResourceGcpVpcSiteInstanceResourceV1().CoreConfigSchema().ImpliedType(),
				Upgrade: statemigration.ResourceGcpVpcSiteInstanceStateUpgradeV1,
				Version: 0,
			},
		},
	}
...
func resourceVolterraGcpVpcSiteCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("disable_encryption"); ok && !encryptionChoiceTypeFound {
		...
		if v.(bool) {
			encryptionChoiceInt := &ves_io_schema_views_gcp_vpc_site.CreateSpecType_DisableEncryption{}
	...
	if v, ok := d.GetOk("enable_encryption"); ok && !isIntfNil(v) && !encryptionChoiceTypeFound {
		...
		encryptionChoiceTypeFound = true
		encryptionChoiceInt := &ves_io_schema_views_gcp_vpc_site.CreateSpecType_EnableEncryption{}
		encryptionChoiceInt.EnableEncryption = &ves_io_schema_views_gcp_vpc_site.GCPDiskEncryption{}
		createSpec.EncryptionChoice = encryptionChoiceInt
		...
		for _, set := range sl {
			if set != nil {
				...
				if v, ok := cs["kms_key_resource_id"]; ok && !isIntfNil(v) {

					encryptionChoiceInt.EnableEncryption.KmsKeyResourceId = v.(string)

				}

				if v, ok := cs["kms_key_ring_id"]; ok && !isIntfNil(v) {

					encryptionChoiceInt.EnableEncryption.KmsKeyRingId = v.(string)
		...
		for _, set := range sl {
			if set != nil {
				...
				if v, ok := cs["k8s_cluster"]; ok && !isIntfNil(v) && !k8SClusterChoiceTypeFound {
					...
					k8SClusterChoiceTypeFound = true
					k8SClusterChoiceInt := &ves_io_schema_views_gcp_vpc_site.GCPVPCVoltstackClusterReplaceType_K8SCluster{}
					k8SClusterChoiceInt.K8SCluster = &ves_io_schema_views.ObjectRefType{}
					siteTypeInt.VoltstackCluster.K8SClusterChoice = k8SClusterChoiceInt
				...
				if v, ok := cs["no_k8s_cluster"]; ok && !isIntfNil(v) && !k8SClusterChoiceTypeFound {
					...
					if v.(bool) {
						k8SClusterChoiceInt := &ves_io_schema_views_gcp_vpc_site.GCPVPCVoltstackClusterReplaceType_NoK8SCluster{}
						k8SClusterChoiceInt.NoK8SCluster = &ves_io_schema.Empty{}
						siteTypeInt.VoltstackCluster.K8SClusterChoice = k8SClusterChoiceInt
					}
...
func resourceVolterraGcpVpcSiteDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = client.DeleteObject(context.Background(), ves_io_schema_views_gcp_vpc_site.ObjectType, namespace, name, opts...)
	if err != nil {
		return fmt.Errorf("error deleting GcpVpcSite: %w", err)
	}
	return nil
