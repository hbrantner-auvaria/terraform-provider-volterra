	...
	if v, ok := d.GetOk("advanced_options"); ok && !isIntfNil(v) {
		...
		for _, set := range sl {
			if set != nil {
				...
				if v, ok := advancedOptionsMapStrToI["max_requests_per_connection"]; ok && !isIntfNil(v) && !maxRequestsPerConnectionChoiceTypeFound {

					maxRequestsPerConnectionChoiceTypeFound = true
					maxRequestsPerConnectionChoiceInt := &ves_io_schema_views_origin_pool.OriginPoolAdvancedOptions_MaxRequestsPerConnection{}

					advancedOptions.MaxRequestsPerConnectionChoice = maxRequestsPerConnectionChoiceInt

					maxRequestsPerConnectionChoiceInt.MaxRequestsPerConnection = uint32(v.(int))

				}

				if v, ok := advancedOptionsMapStrToI["no_request_limit_per_connection"]; ok && !isIntfNil(v) && !maxRequestsPerConnectionChoiceTypeFound {

					maxRequestsPerConnectionChoiceTypeFound = true

					if v.(bool) {
						maxRequestsPerConnectionChoiceInt := &ves_io_schema_views_origin_pool.OriginPoolAdvancedOptions_NoRequestLimitPerConnection{}
