...
func FlattenRequestConstraints(f *ves_io_schema_policy.RequestConstraintType) interface{} {
	...
	if f != nil {
		val := map[string]interface{}{
			...
			"max_cookie_value_size_none":       isEmpty(f.GetMaxCookieValueSizeNone()),
			...
			"max_header_value_size_exceeds":    f.GetMaxHeaderValueSizeExceeds(),
			...
			"max_parameter_name_size_exceeds":  f.GetMaxParameterNameSizeExceeds(),
