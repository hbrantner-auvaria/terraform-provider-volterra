...
func resourceVolterraAwsTgwSite() *schema.Resource {
	return &schema.Resource{
		...
		StateUpgraders: []schema.StateUpgrader{
			{
				Type:    statemigration.ResourceAwsTgwSiteInstanceResourceV1().CoreConfigSchema().ImpliedType(),
				Upgrade: statemigration.ResourceAwsTgwSiteInstanceStateUpgradeV1,
				Version: 0,
			},
		},
	}
	...
	if v, ok := d.GetOk("aws_parameters"); ok && !isIntfNil(v) {
		...
		for _, set := range sl {
			if set != nil {
				...
				if v, ok := awsParametersMapStrToI["disable_encryption"]; ok && !isIntfNil(v) && !encryptionChoiceTypeFound {

					encryptionChoiceTypeFound = true

					if v.(bool) {
						encryptionChoiceInt := &ves_io_schema_views_aws_tgw_site.ServicesVPCType_DisableEncryption{}
						encryptionChoiceInt.DisableEncryption = &ves_io_schema.Empty{}
						awsParameters.EncryptionChoice = encryptionChoiceInt
					}

				}

				if v, ok := awsParametersMapStrToI["enable_encryption"]; ok && !isIntfNil(v) && !encryptionChoiceTypeFound {

					encryptionChoiceTypeFound = true
					encryptionChoiceInt := &ves_io_schema_views_aws_tgw_site.ServicesVPCType_EnableEncryption{}
